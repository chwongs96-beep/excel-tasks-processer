...
<h3>重要限制</h3>
<ul>
  <li><strong>對帳鍵使用「共同欄位名」</strong>（兩檔 Excel 標題列中名稱相同的欄），
    <em>不是</em>映射後的 canonical 欄位。欄名不同請先統一標題或合併後再對帳。</li>
  <li>未選<strong>金額欄</strong>時，只做「鍵是否存在」比對（僅左／僅右），不會產生金額不符。</li>
  <li>若對帳鍵在單邊出現<strong>重複</strong>，merge 結果列數可能膨脹；
    摘要會顯示重複鍵數，請補強鍵或先彙總，避免誤判。</li>
  <li>主畫面「對帳差異」為<strong>摘要預覽（有列數上限）</strong>，完整明細以匯出的 Excel 為準。</li>
</ul>
